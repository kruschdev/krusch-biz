...
def run_fixture_gate(
    ...
    for c in cases:
        ...
        if hit_sections:
            hs0 = hit_sections[0]
            if any(g in hs0 or hs0 in g for g in gold_sections):
                top_1_hit = True
        recalls_at_1.append(1.0 if top_1_hit else 0.0)

        top_5_hit = False
        for hs in hit_sections[:5]:
            if any(g in hs or hs in g for g in gold_sections):
                top_5_hit = True
                break
...
def run_heldout_gate(
    ...
    for c in cases:
        ...
        target_rank = next((idx for idx, hs in enumerate(hit_sections) if target_section in hs), None)
        for mnc in must_not_cite:
            mnc_rank = next((idx for idx, hs in enumerate(hit_sections) if mnc in hs), None)
            if mnc_rank is not None:
                if target_rank is None or mnc_rank < target_rank:
                    superseded_priority_inversions += 1

        top_1 = (target_section in hit_sections[0]) if hit_sections else False
        recalls_at_1.append(1.0 if top_1 else 0.0)

        top_5 = any(target_section in hs for hs in hit_sections[:5])
        recalls_at_5.append(1.0 if top_5 else 0.0)

        rank = next((idx for idx, hs in enumerate(hit_sections, 1) if target_section in hs), 0)
        reciprocal_ranks.append(1.0 / rank if rank > 0 else 0.0)
...
def run_grounding_calibration_matrix(
    ...
    for c in cases:
        ...
        if records and records[0].get("status") == "verified_grounded":
        ...
        if records and records[0].get("failure_mode") == "INVENTED_CLAUSE":
        ...
        if target_slots:
            ...
            if "net_days" in target_slots:
                divergent_draft = f"Pursuant to {sec}, Customer shall pay within ninety (90) days of invoice date ('Net 90')."
                ...
                actual_cap = target_slots["cap_amount"]
                divergent_draft = f"Pursuant to {sec}, liability is capped at ${int(actual_cap * 2):,} for all claims."
            elif "cap_period_months" in target_slots:
                divergent_draft = f"Pursuant to {sec}, warranty protection expires in six (6) months."
            elif "late_interest_pct" in target_slots:
                divergent_draft = f"Pursuant to {sec}, royalty rate shall be 1.5% per month."
            elif "notice_hours" in target_slots:
                divergent_draft = f"Pursuant to {sec}, Vendor shall provide breach notice within seventy-two (72) hours."
            elif "notice_days" in target_slots:
                divergent_draft = f"Pursuant to {sec}, notice must be provided within one hundred (100) days."
            else:
                divergent_draft = f"Pursuant to {sec}, Vendor shall provide thirty (30) days notice."

            _, records, _, _ = verify_commercial_grounding(divergent_draft, retrieved_mock)
            if records and records[0].get("failure_mode") == "DIVERGENT_TERM":
        ...
        if pred:
            ...
            if records and records[0].get("failure_mode") == "SUPERSEDED_TERM":
